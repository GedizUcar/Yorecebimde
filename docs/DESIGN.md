...
> **Reference pattern**: Bu dosya bir referans tasarım sistemi breakdown'ı (Apple) ile başlar.
> Yörecebimde'nin **kendi tasarım sistemi** bu **format'a uygun şekilde** belgelenecek
> (renkler, tipografi, component'ler tabii ki farklı; **yapı ve düşünce sistemi** referans alınır).
>
> **İki amaç:**
> 1. **Pattern / format şablonu** — yeni token eklenince hangi başlıklar altında belgelenmeli (Overview → Colors → Typography → Layout → Elevation → Shapes → Components → Do's/Don'ts → Responsive → Iteration Guide → Known Gaps)
> 2. **Felsefi yönlendirme** — single accent, photography-first (Yörecebimde için ürün fotoğrafı), alternating surface, single shadow, type ladder disiplini
>
> **UI implementasyonunda not:**
> - Token isimleri Yörecebimde'ye özgü olacak (`colors.primary` Yörecebimde marka rengi — Apple'ın `#0066cc` değil; brand color marka kimliği netleşince güncellenir)
> - Tipografi: SF Pro yerine Inter (web), San Francisco (mobil — Expo sistemi)
> - Gıda pazaryeri = Apple kadar minimalist olamayız ama **"ürün başroldedir"** prensibi geçerli
> - Tüm UI work bu dosyaya bakarak yapılacak; eksik / yeni token = burayı güncelle

---

## Reference Pattern Document (Apple Design System Breakdown)

> Aşağıdaki içerik **referans/şablon** amacıyla yapıştırıldı.
> Yörecebimde'nin gerçek token'ları netleştikçe bu içerik **üzerine** veya **paralelinde** yazılacak.
